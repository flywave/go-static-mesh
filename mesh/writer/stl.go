...
func (w *STLWriter) WriteFile(m *mesh.Mesh, path string) error {
...
func (w *STLWriter) Write(m *mesh.Mesh, path string) error {
	return w.WriteFile(m, path)
}

func (w *STLWriter) WriteTo(m *mesh.Mesh, writer io.Writer) error {
	solid := &stl.Solid{
		Name:    "mesh",
		IsAscii: w.ASCII,
	}
